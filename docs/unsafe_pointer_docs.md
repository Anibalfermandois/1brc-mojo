...
The [`UnsafePointer`](/mojo/std/memory/unsafe_pointer/UnsafePointer) type
...
The [`UnsafePointer`](/mojo/std/memory/unsafe_pointer/UnsafePointer)
...
and [`init_pointee_move()`](/mojo/std/memory/unsafe_pointer/UnsafePointer#init_pointee_move)
methods. For example:
...
The [`take_pointee()`](/mojo/std/memory/unsafe_pointer/UnsafePointer#take_pointee)
method moves a pointee from the memory location pointed to by `ptr`. This is
a consuming move. It invokes `__init__(take=)` on the destination value. It leaves
the memory location uninitialized.

The [`destroy_pointee()`](/mojo/std/memory/unsafe_pointer/UnsafePointer#destroy_pointee)
...
Calling [`init_pointee_move_from(self, src)`](/mojo/std/memory/unsafe_pointer/UnsafePointer/#init_pointee_move_from)
...
Calling [`free()`](/mojo/std/memory/unsafe_pointer/UnsafePointer#free) on a
...
(for example, using
[`destroy_pointee()`](/mojo/std/memory/unsafe_pointer/UnsafePointer#destroy_pointee) or
...
an `origin` value of `MutExternalOrigin`. This value represents an origin that
is mutable and doesn't *alias* existing values. For example, it doesn't
point to the memory allocated for any other variable. That means the compiler
won't unnecessarily extend the lifetimes of unrelated values. This memory isn't tracked
by Mojo's lifetime checker and you're responsible for freeing it.
...
[`unsafe_get_as_pointer()`](/mojo/std/python/python_object/PythonObject/#unsafe_get_as_pointer)
...
[`external_call`](/mojo/std/sys/ffi/external_call) function, you can specify
...
def get_foreign_pointer() raises -> UnsafePointer[Int, MutExternalOrigin]:
...
[`byte_swap()`](/mojo/std/bit/bit/byte_swap) function to swap the byte
...
[`load()`](/mojo/std/memory/unsafe_pointer/UnsafePointer#load) and
[`store()`](/mojo/std/memory/unsafe_pointer/UnsafePointer#store) methods for
...
[`strided_load()`](/mojo/std/memory/unsafe_pointer/UnsafePointer#strided_load)
and
[`strided_store()`](/mojo/std/memory/unsafe_pointer/UnsafePointer#strided_store)
...
```mojo
def invert_red_channel(ptr: UnsafePointer[mut=True, UInt8], pixel_count: Int) raises:
...
The [`gather()`](/mojo/std/memory/unsafe_pointer/UnsafePointer#gather) and
[`scatter()`](/mojo/std/memory/unsafe_pointer/UnsafePointer#scatter) methods
