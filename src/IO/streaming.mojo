...
struct FileHandle(Copyable, ImplicitlyCopyable, Movable):
    ...
    def pread(self, buf: UnsafePointer[UInt8, MutUntrackedOrigin], count: Int, offset: Int) -> Int:
...
struct DoubleBuffer:
    ...
    def get_inactive(self, step: Int) -> UnsafePointer[UInt8, MutUntrackedOrigin]:
...
@always_inline
def find_last_newline(ptr: UnsafePointer[UInt8, MutUntrackedOrigin], length: Int) -> Int:
...
struct DoubleBufferedStream[
    ...
    def process_range[
        ...
        var current_pos = range_start
        var step = 0
        var tail_len = 0
        
        # Initial read to prime the pump
        var active_buf = self.buffers.get_active(step)
        var to_read = min(Self.BLOCK_SIZE, range_end - current_pos)
        if to_read <= 0:
            return
        var bytes_read = self.handle.pread(active_buf, to_read, current_pos)
        if bytes_read < 0:
        ...
        while bytes_read > 0:
            ...
            if current_pos >= range_end:
                if tail_len > 0:
                    raise Error("streaming: aligned range ended with a partial row")
                break
